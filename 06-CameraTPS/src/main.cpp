...
void keyCallback(GLFWwindow *window, int key, int scancode, int action,
		int mode); //teclas
void mouseCallback(GLFWwindow *window, double xpos, double ypos); //mouse movimiento
void mouseButtonCallback(GLFWwindow *window, int button, int state, int mod); //mouse botones
void scrollCallback(GLFWwindow *window, double xoffset, double yoffset); //scroll
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Para la cámara en tercera persona no se ocupa, pues la posición va a quedar determinada por la posición del pivote y la distancia
...
bool processInput(bool continueApplication) {
	...
	// La cámara en tercera persona no se puede mover para enfrente, así que esto es solo para primera persona
	...
	//Movimiento de la camara en 3era persona
	offsetX = 0;
	offsetY = 0;
...
void applicationLoop() {
	...
	//Variables para calcular las características de la cámara
	float angleTarget;
	glm::vec3 axis;
	glm::vec3 target;
